...
export default async function loadTexture(url, options) {
  ...
  try {
    ...
    // Ensure image is fully decoded before assigning to texture
    // This prevents "Texture marked for update but image is incomplete" warnings
    ...
    if (options.renderer) {
      ...
      try {
        ...
        // Silently ignore if texture init fails (e.g., context lost)
      }
    }
    return texture
  } catch (err) {
