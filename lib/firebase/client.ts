...
const isValidKey = Boolean(
  apiKey && apiKey.startsWith("AIzaSy") && apiKey.length > 20,
);
...
try {
  ...
  console.warn(
    "Firebase Auth init warning (Configure .env.local for live auth):",
    e,
  );
  const fallbackApp =
    getApps().find((a) => a.name === "fallback") ||
    initializeApp(
      {
