  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = {
        ...
        prizePool: formData.prizePool.startsWith('₹') ? formData.prizePool : `₹${formData.prizePool}`,
        coordinators,
  ...
  return (
    <div className="max-w-6xl mx-auto pb-20">
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-3xl font-bold text-slate-900 tracking-tight">Sabrang Events Manager</h1>
        {!showForm && (
          <button 
      ...
      {!showForm && (
        <div className="flex flex-col md:flex-row gap-4 mb-6">
          ...
          <div className="flex items-center gap-3">
            <label className="text-xs font-black text-slate-400 uppercase tracking-widest whitespace-nowrap">Category:</label>
            <select
      ...
      {showForm && (
        <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-100 mb-12">
          ...
          <div className="flex justify-between items-start mb-8">
            <div>
              <h2 className="text-2xl font-black text-slate-900 uppercase tracking-tight">{editingEvent ? 'Modify Event' : 'Create Event'}</h2>
              <p className="text-slate-500 text-sm">{editingEvent ? 'Update the details for this existing event.' : 'Define a new competition for the festival.'}</p>
            </div>
            <button onClick={resetForm} className="text-slate-400 hover:text-slate-600 font-bold text-sm">Cancel</button>
          </div>

          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="md:col-span-2">
              <label className="block text-xs font-black text-slate-500 uppercase tracking-widest mb-2">Event Title {editingEvent && '(Fixed)'}</label>
              <input 
                type="text" required 
            ...
            <div className="md:col-span-2">
              ...
              <textarea 
                required className="w-full p-4 border border-slate-200 rounded-xl h-32 focus:border-indigo-500 outline-none transition-all"
            ...
            <div>
              <label className="block text-xs font-black text-slate-500 uppercase tracking-widest mb-2">Prize Pool (INR)</label>
              <div className="relative">
                <span className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 font-bold">₹</span>
                <input 
                  type="text" required className="w-full p-4 pl-8 border border-slate-200 rounded-xl focus:border-indigo-500 outline-none"
                  placeholder="50,000"
            ...
            <div>
              <label className="block text-xs font-black text-slate-500 uppercase tracking-widest mb-2">Date & Time</label>
              <input 
                type="datetime-local" required className="w-full p-4 border border-slate-200 rounded-xl focus:border-indigo-500 outline-none"
                value={formData.dateTime} onChange={e => setFormData({...formData, dateTime: e.target.value})}
              />
            </div>

            <div>
              <label className="block text-xs font-black text-slate-500 uppercase tracking-widest mb-2">Venue</label>
              <input 
                type="text" required className="w-full p-4 border border-slate-200 rounded-xl focus:border-indigo-500 outline-none"
            ...
            <div className="md:col-span-2">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 bg-slate-50 p-6 rounded-2xl border border-slate-100">
                <div className="md:col-span-2 flex justify-between">
                  <label className="text-xs font-black text-slate-500 uppercase tracking-widest">Coordinators</label>
                  <span className="text-[10px] text-slate-400 uppercase font-bold italic">Max 2 allowed</span>
                </div>
                
                <div className="space-y-4">
                  <p className="text-sm font-bold text-slate-700 underline decoration-indigo-200">Coordinator 1</p>
                  <input 
                    type="text" placeholder="Full Name" className="w-full p-3 border border-slate-200 rounded-lg bg-white"
                    value={formData.coord1Name} onChange={e => setFormData({...formData, coord1Name: e.target.value})}
                  />
                  <input 
                    type="text" placeholder="Phone Number" className="w-full p-3 border border-slate-200 rounded-lg bg-white"
                    value={formData.coord1Phone} onChange={e => setFormData({...formData, coord1Phone: e.target.value})}
                  />
                </div>

                <div className="space-y-4">
                  <p className="text-sm font-bold text-slate-700 underline decoration-indigo-200">Coordinator 2</p>
                  <input 
                    type="text" placeholder="Full Name" className="w-full p-3 border border-slate-200 rounded-lg bg-white"
                    value={formData.coord2Name} onChange={e => setFormData({...formData, coord2Name: e.target.value})}
                  />
                  <input 
                    type="text" placeholder="Phone Number" className="w-full p-3 border border-slate-200 rounded-lg bg-white"
                    value={formData.coord2Phone} onChange={e => setFormData({...formData, coord2Phone: e.target.value})}
                  />
                </div>
              </div>
            </div>
            ...
            <div className="md:col-span-2">
              <label className="block text-xs font-black text-slate-500 uppercase tracking-widest mb-2">Event Rules & Guidelines</label>
              <textarea 
                required className="w-full p-4 border border-slate-200 rounded-xl h-48 focus:border-indigo-500 outline-none"
            ...
            <div className="md:col-span-2 bg-indigo-50 p-4 rounded-xl flex items-center gap-3">
              ...
              <p className="text-xs text-indigo-700 font-medium">Posters for events are managed directly in the codebase and cannot be edited here.</p>
            </div>
        ...
        <table className="w-full text-left">
          <thead className="bg-slate-50 border-b">
            <tr>
              ...
              <th className="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Prize</th>
          ...
          <tbody className="divide-y divide-slate-100">
            {filteredEvents.map((event) => (
              <tr key={event.id} className="hover:bg-slate-50/50 transition-colors group">
                ...
                <td className="px-6 py-4 text-sm text-slate-500 font-medium">{formatDate(event.dateTime)}</td>
                ...
                <td className="px-6 py-4 text-right space-x-3">
                  <button onClick={() => handleEdit(event)} className="text-indigo-600 hover:text-indigo-800 font-bold text-sm opacity-0 group-hover:opacity-100 transition-opacity">Edit</button>
                  <button onClick={() => handleDelete(event.id!)} className="text-red-600 hover:text-red-800 font-bold text-sm opacity-0 group-hover:opacity-100 transition-opacity">Delete</button>
                </td>
              </tr>
            ))}
            {filteredEvents.length === 0 && (
              <tr>
                <td colSpan={6} className="px-6 py-12 text-center text-slate-500 italic">No events found matching your filters.</td>
              </tr>
            )}
          </tbody>
        </table>
