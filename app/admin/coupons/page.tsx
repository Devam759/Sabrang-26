...
export default function AdminCoupons() {
  ...
  const handleEdit = (coupon: Coupon) => {
    ...
    setFormData({
      ...
      expiryDate: new Date((coupon.expiryDate as any).seconds * 1000).toISOString().slice(0, 16),
        ...
        <div className="bg-white p-8 rounded-2xl shadow-sm border mb-12">
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <input 
                type="text" required className="w-full p-2 border rounded-md font-mono uppercase"
            ...
            <div>
              ...
              <input 
                type="number" required min="0" max="100" className="w-full p-2 border rounded-md"
            ...
            <div className="flex items-center space-x-2 pt-6">
              ...
              />
              <label htmlFor="active" className="text-sm font-medium">Active & Redeemable</label>
            </div>
            <div className="md:col-span-2 flex justify-end space-x-4">
              <button type="button" onClick={resetForm} className="px-6 py-2 text-slate-600">Cancel</button>
              <button type="submit" className="bg-purple-600 text-white px-8 py-2 rounded-lg font-bold">
      ...
      <div className="bg-white rounded-2xl border shadow-sm overflow-hidden text-sm">
        <table className="w-full text-left">
          <thead className="bg-slate-50 border-b">
            <tr>
              <th className="px-6 py-4 font-bold text-slate-500 uppercase">Code</th>
              <th className="px-6 py-4 font-bold text-slate-500 uppercase">Discount</th>
              <th className="px-6 py-4 font-bold text-slate-500 uppercase">Expiry</th>
              <th className="px-6 py-4 font-bold text-slate-500 uppercase">Status</th>
              <th className="px-6 py-4 font-bold text-slate-500 uppercase text-right">Actions</th>
            </tr>
          </thead>
            ...
            {coupons.map((coupon) => (
              <tr key={coupon.id}>
                ...
                <td className="px-6 py-4 text-right space-x-4">
                  <button onClick={() => handleEdit(coupon)} className="text-indigo-600 font-medium">Edit</button>
                  <button onClick={() => handleDelete(coupon.id!)} className="text-red-600 font-medium">Delete</button>
                </td>
              </tr>
            ))}
          </tbody>
