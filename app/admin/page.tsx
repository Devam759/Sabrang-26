...
export default function AdminDashboard() {
    ...
    const fetchStats = async () => {
      try {
        ...
        const registrations = regSnap.docs.map(
          (doc) => doc.data() as Registration,
        );
        const events = eventSnap.docs.map(
  ...
  if (loading)
    return (
      <div className="text-center mt-20 font-bold text-outline animate-pulse">
        VERIFYING AUTH...
      </div>
    );

  const attendanceRatio = Math.round(
    (stats.totalCheckedIn / (stats.totalRegistrations || 1)) * 100,
  );
    ...
    <div className="flex flex-col gap-8">
      ...
      <section className="flex flex-col gap-1">
        ...
        <p className="text-lg text-on-surface-variant font-medium">
          Real-time overview of festival activity and registrations.
        </p>
      </section>
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-surface-container-lowest p-6 rounded-xl card-shadow border border-outline-variant flex flex-col justify-between h-44 hover-scale transition-all">
          ...
          <div className="flex items-end justify-between">
            ...
            <span className="text-primary-container font-bold text-xs uppercase tracking-wider bg-primary/5 px-2 py-1 rounded">
              Live Tracking
            </span>
          </div>
        </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-surface-container-low">
                  <th className="px-6 py-4 text-[10px] font-bold text-on-secondary-container uppercase tracking-widest">
                    Event Name
                  </th>
                  <th className="px-6 py-4 text-[10px] font-bold text-on-secondary-container uppercase tracking-widest text-center">
                    Regs
                  </th>
                  <th className="px-6 py-4 text-[10px] font-bold text-on-secondary-container uppercase tracking-widest text-right">
                    Status
                  </th>
                </tr>
              </thead>
                ...
                {dataLoading
                  ? [1, 2, 3, 4].map((i) => (
                      <tr key={i}>
                        <td
                          colSpan={3}
                          className="px-6 py-8 animate-pulse bg-surface-container-low/20"
                        ></td>
                      </tr>
                    ))
                  : stats.eventBreakdown.slice(0, 5).map((event, idx) => (
                      ...
                      >
                        <td className="px-6 py-6 text-sm font-bold text-on-surface">
                          {event.title}
                        </td>
                        <td className="px-6 py-6 text-sm text-on-surface-variant text-center font-bold">
                          {event.count}
                        </td>
        ...
        <section className="flex flex-col gap-6">
          ...
          <div className="relative overflow-hidden bg-inverse-surface rounded-xl p-6 card-shadow text-on-primary-container h-[26rem] flex flex-col justify-between group">
            ...
            <div className="relative z-10">
              ...
              <p className="text-sm text-primary-fixed-dim font-medium">
                Instant check-in for on-site participants.
              </p>
            </div>
            <div className="relative z-10 flex flex-col gap-6">
              <div className="flex justify-center py-8 bg-white/5 border border-white/10 backdrop-blur-sm">
                <svg
                  className="w-20 h-20 text-white opacity-40"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                  strokeWidth={1.5}
          ...
          <div className="grid grid-cols-1 gap-4">
            <Link
              href="/admin/events"
              className="bg-surface-container-highest p-4 rounded-lg hover:bg-primary hover:text-white transition-all group flex items-center justify-between hover-scale"
            >
