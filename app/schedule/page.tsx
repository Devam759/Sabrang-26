...
export const metadata: Metadata = {
  ...
  description: 'Complete 3-day timeline and event schedule for Sabrang 2026 at JK Lakshmipat University. Track workshops, prelims, finals, and pro-shows.',
  keywords: [
...
const scheduleSchema = {
  ...
  description: 'Official 3-day event timeline for Sabrang 2026 at JK Lakshmipat University.',
...
export default function SchedulePage() {
  const schedule = {
    ...
    'Day 3 - November 8, 2026': [
      ...
      { time: '7:00 PM', event: 'Pro-Show Concert', venue: 'Main Stage', type: 'Entertainment' },
    ],
  };
  ...
  const getTypeColor = (type: string) => {
    const colors: Record<string, string> = {
      ...
      Technical: 'bg-emerald-900/60 text-emerald-300 border border-emerald-500/30',
    ...
    <>
      ...
      <div className="max-w-6xl mx-auto space-y-12 py-10 px-4">
        ...
        <section className="text-center space-y-6">
          <h1 className="text-4xl md:text-6xl font-black text-white tracking-tight uppercase">Event Schedule</h1>
          <p className="text-lg md:text-xl text-white/70 max-w-3xl mx-auto">
            Three days of non-stop innovation, culture, and high-energy pro-shows.
          </p>
        ...
        {Object.entries(schedule).map(([day, events]) => (
          <section key={day} className="space-y-6">
            <h2 className="text-2xl md:text-3xl font-black text-white tracking-tight border-l-4 border-purple-500 pl-4">{day}</h2>
            <div className="bg-neutral-900/80 backdrop-blur-xl rounded-3xl border border-white/10 shadow-2xl overflow-hidden">
              <div className="divide-y divide-white/10">
                {events.map((item, index) => (
                  <div key={index} className="p-6 hover:bg-white/5 transition-colors grid grid-cols-1 md:grid-cols-4 gap-4 items-center">
                    <div className="font-bold text-white text-lg">{item.time}</div>
                    <div className="md:col-span-2">
                      <div className="font-bold text-white text-lg">{item.event}</div>
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <ul className="text-sm text-white/70 space-y-1 list-disc list-inside">
                <li>Schedule is subject to minor adjustments. Please check for updates regularly.</li>
                <li>Participants must report 30 minutes before their event slot.</li>
                <li>Venue notifications will be communicated via official announcements.</li>
              </ul>
            </div>
          </div>
        </section>
